...
WRECK_VERSION = '1.0.0-first-pass-mod'
...
if 'bw' in sys.argv:
    ...
    try:
        import colorama
        colorama.init()
        COLORAMA = ('\x1b[0m', '\x1b[31m', '\x1b[32m', '\x1b[33m',
                    '\x1b[34m', '\x1b[35m', '\x1b[36m', '\x1b[37m')
    except:
        COLORAMA = ('', '', '', '', '', '', '', '')


# Where the compiler will write new iteration ID-files.
write_id_files = "ID_%s.py"
# Set to true to display compiler performance data by default.
show_performance_data = False
# How to name export files (only used for some debugging purposes).
export_filename = '%s.txt'

WRECK.time_started = gettime()


print
print '{2}*** Warband Refined & Enhanced Compiler Kit (W.R.E.C.K.) version {version!s} ***{0}'.format(
    *COLORAMA, version=WRECK_VERSION)
print 'Please report errors, problems and suggestions at {5}http://lav.lomskih.net/wreck/{0}'.format(
    *COLORAMA)
print
  ...
  # |
    ...
    try:
        ...
        print '{1}FAILED.\nMODULE `{module!s}` ERROR:\n{error!s}{0}'.format(
            *COLORAMA, module=WRECK.current_module, error=(e.formatted() if isinstance(e, MSException) else formatted_exception()))
        if isinstance(e, TypeError) and (('object is not callable' in e.message) or ('indices must be integers' in e.message)):
            ...
            print '{6}  Compiler hint: this error is typically caused by a missing comma.\n  Please check that tuples are followed by commas in `{path!s}` above line {line}:\n\n    {5}{code!s}{0}'.format(
                *COLORAMA, path=path_split(error_info[0])[1], line=error_info[1], code=error_info[3])
    ...
    try:
        ...
        for plugin, required_by in WRECK.requirements.iteritems():
            if plugin not in WRECK.plugins:
                prereq_errors.append('Plugin %s not imported but required by %s.' % (
                    plugin, ', '.join(required_by)))
        if prereq_errors:
            raise MSException('\r\n'.join(prereq_errors))
        ...
        for plugin in WRECK.plugins:
            ...
            if injections:
                for inj_name, inj_elements in injections.iteritems():
                    WRECK.injections.setdefault(
                        inj_name, []).extend(inj_elements)
                    # WRECK.warnings.append('Injection: %d elements for `%s` in `%s`' % (len(inj_elements), inj_name, plugin))
    except Exception, e:
        print '{1}FAILED.\nPLUGIN `{module!s}` ERROR:\n{error!s}{0}'.format(
            *COLORAMA, module=plugin, error=(e.formatted() if isinstance(e, MSException) else formatted_exception()))
...
# |
    ...
    try:
        for entity_name, entity_def in parsers.iteritems():
            ...
            get_globals()[entity_name] = check_syntax(get_globals()[entity_name], [
                entity_def['parser']], entity_def.get('uid', 0))
        WRECK.current_module = None
    except Exception, e:
        print '{1}FAILED.\nMODULE `{module!s}` ERROR:\n{error!s}{0}'.format(
            *COLORAMA, module=entity_name, error=(e.formatted() if isinstance(e, MSException) else formatted_exception()))
    ...
    try:
        ...
        if undefined:
            raise MSException('undeclared identifiers found in module source:\n * %s' % ('\n * '.join(
                ['%s (referenced by \'%s\')' % (name, '\', \''.join(refs)) for name, refs in undefined])))
    except Exception, e:
        print '{1}FAILED.'.format(*COLORAMA)
        if isinstance(e, MSException):
            print 'MODULE ERROR:\n{error!s}{0}'.format(
                *COLORAMA, error=e.formatted())
        else:
            print 'COMPILER INTERNAL ERROR:\n{error!s}{0}'.format(
                *COLORAMA, error=formatted_exception())
        ...
        for plugin in WRECK.plugins:
            ...
            if processor:
                try:
                    ...
                    raise MSException(
                        'Error in %r pre-processor script.' % plugin, formatted_exception())
        # Compiling...
        stage = 2
        for entity_name, entity_def in parsers.iteritems():
            stage = 3
            ...
            for index in xrange(len(entities)):
                entities[index] = entity_def['processor'](
        ...
        if isinstance(e, MSException):
            ...
            if stage == 1:
                print 'PLUGIN {module!s} PREPROCESSOR ERROR:\n{error!s}{0}'.format(
                    *COLORAMA, module=plugin, error=e.formatted())
            elif stage == 3:
                print 'MODULE {module!s} ENTITY #{index} COMPILATION ERROR:\n{error!s}{0}'.format(
                    *COLORAMA, module=entity_name, index=index, error=e.formatted())
            elif stage == 4:
                print 'MODULE {module!s} AGGREGATOR ERROR:\n{error!s}{0}'.format(
                    *COLORAMA, module=entity_name, error=e.formatted())
            elif stage == 5:
...
# |
    ...
    if WRECK.generate_item_modifiers:
        export['item_modifiers'] = 'Data/item_modifiers.txt'
    if WRECK.generate_ui_strings:
        export['ui_strings'] = 'Languages/en/ui.csv'
    if WRECK.generate_user_hints:
        export['user_hints'] = 'Languages/en/hints.csv'
    ...
    try:
        for entity_name, filename in export.iteritems():
            ...
            folder = ('%s/%s' % (WRECK.destination,
                      filename[0])) if filename[0] else WRECK.destination
            if filename[0] and not(path_exists(folder)):
                makedirs(folder)
            with open('%s/%s' % (folder, filename[1]), 'w+b') as f:
                f.write(contents)
    except Exception, e:
        print '{1}FAILED.\nCOMPILER INTERNAL ERROR WHILE WRECKING {module!s}:\n{error!s}{0}'.format(
            *COLORAMA, module=entity_name, error=formatted_exception())
...
if ('error' in sys.argv) or ('errors' in sys.argv):
    error_reporting_level = 1
if ('error' in sys.argv) or ('warnings' in sys.argv):
    error_reporting_level = 2
if ('notice' in sys.argv) or ('notices' in sys.argv):
    error_reporting_level = 3
...
if show_performance_data and WRECK.time_loaded:
    ...
    print 'Use {5}show_performance_data = False{0} directive in {5}module_info.py{0} file to disable.'.format(
        *COLORAMA)
    print
    if WRECK.time_loaded:
        print '    %.03f sec spent to load module data.' % (
            WRECK.time_loaded - WRECK.time_started)
    if WRECK.time_plugins:
        print '    %.03f sec spent to load plugins.' % (
            WRECK.time_plugins - WRECK.time_loaded)
    if WRECK.time_syntax:
        print '    %.03f sec spent to check module syntax.' % (
            WRECK.time_syntax - WRECK.time_plugins)
    if WRECK.time_identifiers:
        print '    %.03f sec spent to allocate identifiers.' % (
            WRECK.time_identifiers - WRECK.time_syntax)
    if WRECK.time_compile:
        print '    %.03f sec spent to compile module.' % (
            WRECK.time_compile - WRECK.time_identifiers)
    if WRECK.time_export:
        print '    %.03f sec spent to export module.' % (
            WRECK.time_export - WRECK.time_compile)
    print
    print '    >>> %.03f sec total time spent. <<<' % (
        gettime() - WRECK.time_started)
    print
if 'wait' in sys.argv:
